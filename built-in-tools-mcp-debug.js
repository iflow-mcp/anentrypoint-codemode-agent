...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  console.error('[DEBUG] Tool name:', name, 'Args:', JSON.stringify(args, null, 2));
...
async function main() {
  console.error('[DEBUG] Starting built-in tools MCP server...');
  const transport = new StdioServerTransport();
  console.error('[DEBUG] Transport created, connecting...');
  await server.connect(transport);
  console.error('[DEBUG] Server connected and running on stdio');
}
